...
def identity_dao(identity_request: IdentityRequestModel):
    ...
    # Find all objects
    email_contacts = Contact.query.filter(Contact.email == email).all()
    phoneNumber_contacts = Contact.query.filter(Contact.phoneNumber == phoneNumber).all()
    ...
    """
    min_email_contact = Contact.query.filter(Contact.email == email).order_by(Contact.id).first()
    min_phone_contact = Contact.query.filter(Contact.phoneNumber == phoneNumber).order_by(Contact.id).first()
